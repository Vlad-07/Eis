...
#include "Eis/Core/Log.h"
	...
	#define EIS_CORE_ASSERT(x) { if(!(x)) { EIS_DEBUGBREAK(); } }
	#define EIS_ASSERT(x, ...) { if(!(x)) { EIS_CRITICAL("Assertion Failed: {}", __VA_ARGS__); EIS_DEBUGBREAK(); } }
	#define EIS_ASSERT(x) { if(!(x)) { EIS_DEBUGBREAK(); } }
#else
