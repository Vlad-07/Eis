...
namespace Eis
{
		...
		: m_Position(pos)
	{
		...
		float angularInertia = !descriptor.Static ? 0.5f * m_Properties.Mass * radius * radius : 0.0f;
		...
		: m_Position(pos), m_Rotation(rotation)
	{
		...
		EIS_ASSERT(verts.size() >= 3, "Invalid vertex count: {}!", verts.size()); // TODO: does the engine work with a line(2 vertices) ?
		...
		{
			float di = verts[i].x * verts[(i + 1) % verts.size()].y - verts[(i + 1) % verts.size()].x * verts[i].y;
